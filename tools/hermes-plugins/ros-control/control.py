...
def handle_ros_start(raw_args, repo_dir=DEFAULT_REPO_DIR):
    returncode, stdout, stderr = run_control(["-Command", "start"], repo_dir)
    if returncode == 0:
...
def handle_ros_status(raw_args, repo_dir=DEFAULT_REPO_DIR):
    returncode, stdout, stderr = run_control(
        ["-Command", "status", "-Json"], repo_dir
    )
    if returncode == 0:
        return stdout.strip()
...
def handle_ros_stop(raw_args, repo_dir=DEFAULT_REPO_DIR):
    returncode, stdout, stderr = run_control(["-Command", "stop"], repo_dir)
    if returncode == 0:
