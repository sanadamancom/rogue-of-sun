 ...
 */
export function extractMonsterHouseCandidateRooms(map: GameMap, start: Vec2, exit: Vec2, extraExcludedRoomIndex?: number | null): number[] {
  ...
  for (let i = 0; i < map.rooms.length; i++) {
    if (i === startRoomIndex || i === exitRoomIndex || i === extraExcludedRoomIndex) continue;
    candidates.push(i);
  }
  return candidates;
}
...
export function buildMonsterHouseFloorState(
  ...
  exit: Vec2,
  rng: () => number,
  extraExcludedRoomIndex?: number | null,
): MonsterHouseState {
  if (!isMonsterHouseEligibleFloor(depth, leg)) return null;

  const candidates = extractMonsterHouseCandidateRooms(map, start, exit, extraExcludedRoomIndex);
