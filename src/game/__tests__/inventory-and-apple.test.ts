...
describe('inventory overlay controls (Tab/Escape/Arrow/Enter)', () => {
  ...
  it('opening resets the selected index to 0', () => {
    const state = freshState({ inventory: { apple: 3, sword: 0, armor: 0, spear: 0, hammer: 0, sun_fruit: 0, solar_gun: 0, sol_enchantment: 0, chocolate: 0 }, selectedItemIndex: 5 });
    toggleInventory(state);
  ...
  it('inventory display includes positive-count items', () => {
    const state = freshState({ inventory: { apple: 2, sword: 0, armor: 0, spear: 0, hammer: 0, sun_fruit: 0, solar_gun: 0, sol_enchantment: 0, chocolate: 0 } });
