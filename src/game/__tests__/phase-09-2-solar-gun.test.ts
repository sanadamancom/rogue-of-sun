...
describe('solar gun placement (Phase 15.4b random ground item generation)', () => {
  it('is a valid candidate on floor 1, floor 2, and floor 3 (cumulative pool, no longer floor-1-only)', () => {
    expect(getGroundItemPoolForFloor(1, 'descent')).toContain('solar_gun');
    expect(getGroundItemPoolForFloor(2, 'descent')).toContain('solar_gun');
    expect(getGroundItemPoolForFloor(3, 'descent')).toContain('solar_gun');
  });
