...
### `resolveRegularReinforcement`のenemy drop production wiringバグ修正（human decision, 2026-08-22）

`24.6c5`slice 4完了後、layer A generation auditの§18.3チェックリスト残項目（増援周期、enemy drop候補、Otenco関連）を次slice（想定slice 5）で追加する前提を確認する過程で、`resolveRegularReinforcement`（`src/game/turn.ts`）が、増援spawnのcap・species pool・levelについて、`24.6c2b`が定義したcanonical depth table（`getEnemyPopulationForDepth`／`getEligibleEnemySpeciesForDepth`／`getEnemyLevelBandForDepth`）ではなく、legacy 3F固定値（`ENEMY_COUNT_BY_FLOOR`／`ENEMY_COUNT_PER_FLOOR`、`getEnemyPoolForFloor`によるfirst-appearance-floor pool、level常に1固定）を使っていることが判明した。`cadenceTurns`は`getReinforcementRule`（`src/game/reinforcement.ts`）経由で既に`getEnemyPopulationForDepth`のcanonical depth tableを使っており対象外。26F production run（`createInitialState`の`{ totalFloors: 26, runDepthTier: 'deep' }`のようなdeep run config）でdepth 4以降に増援が発生すると、cap・出現種・levelがdesign doc §7／§8のcanonical tableと乖離する。

これは`24.6c5`のbalance判断ではなく、`24.6c4e`が完了させたはずのproduction wiring（`24.6c2b`のcanonical depth tableをproduction spawn pathへ接続する範囲、本節冒頭§19および「24.6c4eのスコープ確定」参照）の実装漏れ（bug）であるため、userへ判断を仰いだ結果、以下のとおり確定した（Decision Baseブランチ`phase-24-6b2a2-availability-regression-coverage` commit `f45120d`）。

1. **A案を採用する。** `resolveRegularReinforcement`を、`24.6c4e`のproduction wiring bug fixとして修正する。`24.6c5`のbalance/定数調整taskとは独立したbug fixであり、修正はaudit続行（増援周期・enemy drop候補チェックの追加）に先行して行う。
2. **修正方針：** 増援spawnのcap・種族・levelを解決するpure helperを`resolveEnemySpawnsForDepth`と共有する形で新設し（単発1体分の抽選ロジックを`resolveEnemySpawnsForDepth`から切り出し、両者から呼ぶ薄い共有helperとする。canonical dataを二重管理しない）、`resolveRegularReinforcement`は`state.runDepthTier !== 'short'`のときだけ、この共有helperと`getEnemyPopulationForDepth(state.floor).initialEnemyCount`を使う。`state.runDepthTier === 'short'`（`DEFAULT_RUN_CONFIG`の3F run）のときは、既存のlegacy cap（`ENEMY_COUNT_BY_FLOOR`／`ENEMY_COUNT_PER_FLOOR`）・legacy pool（`getEnemyPoolForFloor`）・level常に1固定を完全に維持する。`totalFloors >= 26`のようなfeature flag／thresholdは新規に作らない。増援のcadence・ordinal・候補セル選定・RNG seed導出ロジック自体は変更しない（cap・species・levelの3入力のみ差し替える）。
3. **regression test方針：** 複数seedについて、(a) `runDepthTier === 'short'`での増援spawn（cap・出現種・level・RNG消費・イベント）が修正前と完全に一致すること（byte-identical）、(b) `{ totalFloors: 26, runDepthTier: 'deep' }`のようなdeep run configでdepth 4以降の増援capが`getEnemyPopulationForDepth(depth).initialEnemyCount`と一致し、出現種が`getEligibleEnemySpeciesForDepth(depth)`のdepth窓外に出ないこと、level分布が`getEnemyLevelBandForDepth`のLv2/Lv3帯を反映すること、を確認する。descent・ascent両legで確認する。
4. **`24.6c5`のslice番号・スコープ自体は変更しない。** 本bug fixはslice 5（増援周期・enemy drop候補・Otenco関連checkの追加）着手前の前提修正として扱う。
